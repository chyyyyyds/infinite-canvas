...
+ [新增] 支持由同源 New API 宿主安全选择并注入生图 API Key，避免凭据出现在 URL 中。
+ [调整] New API 宿主渠道跟随所选 API Key 自动命名并拉取可用模型，协议、接口地址和凭据由宿主管理且不可编辑。
+ [修复] 嵌入 New API 时，生图与视频生成记录按登录用户隔离，不再被同一浏览器中的其他账号看到。
+ [修复] 视频工作台切换宿主 API Key 后只展示该渠道模型，并按 KOKO 模型同步清晰度、比例和时长，同时避免模型下拉框焦点递归报错。
